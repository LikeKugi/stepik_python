...
# целое число бросалось исключение NonPositiveError и число не добавлялось, а при попытке добавить
...
class PositiveList(list):
    def append(self, __object) -> None:
        if __object > 0 and isinstance(__object, int):
            x = super(PositiveList, self).append(__object)
        else:
