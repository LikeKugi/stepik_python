...
Выведите одно целое число, равное максимальному количеству задач, которое может решить Лимак так,
чтобы прибыть на новогоднюю вечеринку ровно в полночь или раньше.
"""

max_time = 4 * 60
n, k = map(int, input().split())
time_to_solve = max_time - k
i = 0
while time_to_solve > 0 and i < n:
    if time_to_solve - (5 * (i + 1)) >= 0:
        i += 1
        time_to_solve -= 5 * i
    else:
        break
